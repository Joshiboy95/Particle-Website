...
- **Adaptive particle-count calibration:** on every fresh load (or after Reset), `S.count` always starts at `COUNT_CALIB_START` (30,000), independent of `DEFAULTS.count`. `_tickCountCalibrator(now, instFps)` (called every frame from `animate()`) measures the raw, *unsmoothed* per-frame fps (not the `_fps` EMA — an EMA would average away exactly the short stutters this is meant to catch) in rolling `COUNT_CALIB_WINDOW`-long (4s) windows. A window only counts as healthy if its average is ≥55, its 10th-percentile low is ≥50, **and** `physK<=2` — the `physK` gate matters because the performance governor escalating to 3/4 (see above) can keep raw fps looking fine while each particle's own motion gets visibly choppier (fewer physics updates per particle), so a "healthy" fps reading at physK>2 is exactly the smoothness regression the user cares about, not a pass. On a healthy window it grows `S.count` by `COUNT_CALIB_STEP` (10,000, capped at `COUNT_CALIB_MAX` = 200,000), keeps probing, and resets the consecutive-failure counter (`_countCalib.fails`). On a failed window it increments `_countCalib.fails` and, as long as that's below `COUNT_CALIB_MAX_FAILS` (3), just tries again at the same level next window — a single bad window (e.g. a brief loading hiccup) doesn't immediately cost the higher particle count its chance. Only after 3 consecutive failed windows at the same level does it roll `S.count` back down by one step (floored at `COUNT_CALIB_START`) and disable itself (`_countCalib.active`) for the rest of the session — this is what finds the actual highest sustainable particle count per device instead of a single fixed default. Permanently disabled (`_countCalibStop()`) the moment the user sets a particle count themselves, by any means: dragging/editing the count slider (`disableAnimOnTouch`), clicking a Schnellstart preset, Import, or Surprise Me — from that point on, the user's chosen value simply stays put. Re-armed (back to 30,000, `active:true`, `fails:0`) only by the Reset button, since Reset restores factory/default behaviour and the calibrator *is* the default. Live status shown in the perf-display readout (`· Kalibriere Partikelanzahl…`) while active.
...
| `_tickCountCalibrator(now, instFps)` | Ramps `S.count` up from `COUNT_CALIB_START` in 4s windows while raw fps + `physK<=2` stay healthy; tolerates up to `COUNT_CALIB_MAX_FAILS` (3) consecutive bad windows at a level before rolling back + disabling itself; called every frame from `animate()` |
